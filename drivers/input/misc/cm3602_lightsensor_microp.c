...
#define E(x...) printk(KERN_ERR "CM3602_LIGHT ERROR: " x)
...
static int lightsensor_probe(struct platform_device *pdev)
{
	...
	if (!li->client) {
		E("%s: can't get microp i2c client\n", __func__);
		return -1;
	}
	...
	if (ret < 0) {
		E("%s: can not register input device\n",
				__func__);
		return ret;
	}
	...
	if (ret < 0) {
		E("%s: can not register misc device\n",
				__func__);
		return ret;
	}
	...
	if (ret < 0) {
		E("%s: request_irq(%d) failed for (%d)\n",
			__func__, irq, ret);
		return ret;
	}
	...
	if (li->ls_config->ls_gpio_on) {
		ret = gpio_request(li->ls_config->ls_gpio_on,
				"microp_i2c");
		if (ret < 0) {
			E("request gpio ls failed\n");
			return ret;
		}
		ret = gpio_direction_output(li->ls_config->ls_gpio_on, 0);
		if (ret < 0) {
			E("gpio_direction_output ls failed\n");
			return ret;
		}
	}
	ls_power(1);
